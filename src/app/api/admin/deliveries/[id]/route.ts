...
export async function PATCH(
  ...
  try {
    ...
    if (status !== undefined) {
      ...
      if (status === DeliveryStatus.IN_PROGRESS && existingDelivery.order.status !== OrderStatus.OUT_FOR_DELIVERY) {
        await prisma.order.update({
          where: { id: existingDelivery.orderId },
          data: { status: OrderStatus.OUT_FOR_DELIVERY },
        });
        
        // Add status history entry if not already OUT_FOR_DELIVERY
